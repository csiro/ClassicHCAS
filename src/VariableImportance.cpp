...
// Descending-probability ordering, identical to the selection used by
// benchmark() and reference_use() so the retained reference set matches.
...
// Per-variable, locally standardised departure attribution.
//
...
// variable the importance is the signal-to-noise ratio
//
//   importance_v = signal_v / (noise_v + epsilon)
//
// where, with normalised distance weights w_k summing to W,
//
//   signal_v = sum_k w_k |obs_target,v - obs_ref,k,v| / W
//   noise_v  = sum_k sum_l w_k w_l |obs_ref,k,v - obs_ref,l,v| / W^2
//
// i.e. signal is the weighted mean absolute departure of the target from its
// references on variable v, and noise is the weighted mean absolute difference
// among the references themselves on the same variable (a distance-weighted
// Gini mean difference). Both stay in the L1 geometry of the engine, so the
// per-variable signals sum to the weighted total observed departure that the
// reference-density lookup consumes. The output has one row per target cell and
// one column per RS variable; NaN rows mark cells with missing observed values
// or with no usable references.
// [[Rcpp::export]]
    ...
    Rcpp::Nullable<Rcpp::NumericVector> boost = R_NilValue)
{
    ...
    if (output != "importance" && output != "signal") {
        Rcpp::stop("'output' must be 'importance' or 'signal'.");
    }
    const bool want_signal = (output == "signal");
    if (xy_stats.size() != 4) {
    ...
    // One row per target cell, one column per RS variable. Filled in parallel
    // (each thread owns its own rows) then copied into the R matrix afterwards.
    const double NA_VAL = std::numeric_limits<double>::quiet_NaN();
    std::vector<double> importance(static_cast<size_t>(nr) * nvar, NA_VAL);
    ...
    for (int i = 0; i < nr; ++i)
    {
        ...
        std::vector<double> ref_obs(static_cast<size_t>(n_keep) * nvar);
        double max_log_weight = -std::numeric_limits<double>::infinity();
        const double log_boost = std::isnan(boost_factor)
            ? 0.0
            : std::log(boost_factor);
        ...
        for (int k = 0; k < n_keep; ++k) {
            const int candidate = order[k];
            const int site = candidate_sites[candidate];
            const double log_weight = log_distance_weight(
                predicted_distances[candidate],
                lambda,
                kernel_method
            ...
            for (int v = 0; v < nvar; ++v) {
                ref_obs[static_cast<size_t>(k) * nvar + v] =
                    static_cast<double>(samples(site, obs_start + v));
            }
        }
        ...
        const size_t base = static_cast<size_t>(i) * nvar;

        for (int v = 0; v < nvar; ++v) {
            const double target_v = static_cast<double>(cell_obs(v));

            // signal: weighted mean absolute departure of target from references
            double signal = 0.0;
            for (int k = 0; k < n_keep; ++k) {
                signal += weights[k] *
                    std::abs(target_v - ref_obs[static_cast<size_t>(k) * nvar + v]);
            }
            signal *= inv_w;

            if (want_signal) {
                // raw per-variable departure contribution; per cell these sum
                // to the weighted total observed departure feeding the lookup.
                importance[base + v] = signal;
                continue;
            }

            // noise: weighted mean absolute difference among the references
            // (distance-weighted Gini mean difference); zero when n_keep == 1.
            double noise = 0.0;
            for (int k = 0; k < n_keep; ++k) {
                const double rkv = ref_obs[static_cast<size_t>(k) * nvar + v];
                const double wk = weights[k];
                for (int l = k + 1; l < n_keep; ++l) {
                    noise += wk * weights[l] *
                        std::abs(rkv - ref_obs[static_cast<size_t>(l) * nvar + v]);
                }
            }
            noise *= 2.0 * inv_w2; // count ordered pairs (k,l) and (l,k)

            importance[base + v] = signal / (noise + epsilon);
        }
    }

    Rcpp::NumericMatrix out(nr, nvar);
    for (int i = 0; i < nr; ++i) {
        const size_t base = static_cast<size_t>(i) * nvar;
        for (int v = 0; v < nvar; ++v) {
            out(i, v) = importance[base + v];
        }
    }
    return out;
}
