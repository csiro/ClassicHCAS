...
namespace kdt
{
    ...
    public:
         ...
         */
        int nnSearch(const PointT &query, double *minDist = nullptr, int distanceType = 2) const
        {
            int guess;
            double _minDist = std::numeric_limits<double>::max();

            nnSearchRecursive(query, root_, &guess, &_minDist, distanceType);
         ...
         */
        std::vector<int> knnSearch(const PointT &query, int k, int distanceType = 2) const
        {
            KnnQueue queue(k);
            knnSearchRecursive(query, root_, queue, k, distanceType);
         ...
         */
        std::vector<int> radiusSearch(const PointT &query, double radius, int distanceType = 2) const
        {
            std::vector<int> indices;
            radiusSearchRecursive(query, root_, indices, radius, distanceType);
            return indices;
        }
    ...
    private:
        ...
        static double distance(const PointT &p, const PointT &q, int distanceType)
        {
            if (distanceType == 1)
            { // L1 distance
                double dist = 0;
         ...
         */
        void nnSearchRecursive(const PointT &query, const Node *node, int *guess, double *minDist, int distanceType) const
        {
            if (node == nullptr)
                return;
            ...
            const double dist = distance(query, train, distanceType);
            if (dist < *minDist)
            {
                *minDist = dist;
                *guess = node->idx;
            ...
            nnSearchRecursive(query, node->next[dir], guess, minDist, distanceType);
         ...
         */
        void knnSearchRecursive(const PointT &query, const Node *node, KnnQueue &queue, int k, int distanceType) const
        {
            if (node == nullptr)
                return;
            ...
            const int axis = node->axis;
            const int dir = query[axis] < train[axis] ? 0 : 1;
            knnSearchRecursive(query, node->next[dir], queue, k, distanceType);
        ...
        /** @brief Searches neighbors within radius.
         */
        void radiusSearchRecursive(const PointT &query, const Node *node, std::vector<int> &indices, double radius, int distanceType) const
        {
            if (node == nullptr)
                return;
            ...
            const int axis = node->axis;
            const int dir = query[axis] < train[axis] ? 0 : 1;
            radiusSearchRecursive(query, node->next[dir], indices, radius, distanceType);
